...
class CustomBannersPlugin extends CBP_GoldPlugin
{
	...
	function add_stylesheets_and_scripts()
	{
		...
		/*
		if(isValidCBKey()){
			$proCssUrl = plugins_url( 'assets/css/wp-banners-pro.css' , __FILE__ );
			$this->add_stylesheet('wp-banners-pro-css',  $proCssUrl);
		}
		
		//theme stylesheets to admin for theme preview section
		$this->add_admin_stylesheet('wp-banners-css',  $cssUrl);
		$this->add_admin_stylesheet('wp-banners-pro-css',  $proCssUrl);
		*/
		...
		//$this->add_admin_script('cycle2',  $jsUrl, array( 'jquery' ),	false, true);
